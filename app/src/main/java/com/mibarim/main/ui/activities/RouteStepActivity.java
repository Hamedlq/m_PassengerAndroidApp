...
public class RouteStepActivity extends BootstrapActivity implements AddMapFragment.OnMapClickedListener, StepAddressFlagFragment.AddressFlagFragmentListener, DriverPassengerStep.DriverPassFragmentListener, TimeStep.TimeStepFragmentListener, StepperLayout.StepperListener,StepSrcDstFragment.SrcDstStepFragmentListener {
    ...
    private void checkAuth() {
        new SafeAsyncTask<Boolean>() {
            ...
            public Boolean call() throws Exception {
                final AuthenticateService svc = serviceProvider.getService(RouteStepActivity.this);
                return svc != null;
            }
    ...
    public void onBackPressed() {
        ...
        if (fragment != null) {
            ((MainRouteStepFragment) fragment).backExitButton();
        }
    }
    ...
    public void onNetworkErrorEvent(NetworkErrorEvent event) {
        ...
        Toaster.showLong(RouteStepActivity.this, getString(R.string.network_error), R.drawable.toast_warn);
    }
    ...
    public String getInfoText() {
        return getString(R.string.main_add_sher);
    }
    ...
    private void getPlaceDetail(final String placeId) {
        new SafeAsyncTask<Boolean>() {
            ...
            public Boolean call() throws Exception {
                ...
                if (getRouteStates() == AddRouteStates.SelectDestinationState ||
                        getRouteStates() == AddRouteStates.SelectGoWorkState) {
                    DetailPlaceResult response = addressService.getPlaceDetail(placeId);
            ...
            protected void onSuccess(final Boolean res) throws Exception {
                ...
                } else if (getRouteStates() == AddRouteStates.SelectDestinationState
...
//                    dstBriefAdd = bAdd.get(0);
    ...
    private boolean validateRouteRequest(RouteRequest routeRequest) {
        ...
        if (routeRequest.SrcLatitude == null || routeRequest.SrcLatitude.equals("") ||
            ...
            Toaster.showLong(RouteStepActivity.this, getString(R.string.src_dest_not_set), R.drawable.toast_warn);
            res = false;
        }
        if (routeRequest.TheTimeString() == null || routeRequest.TheTimeString().equals("")) {
            Toaster.showLong(RouteStepActivity.this, getString(R.string.time_not_set), R.drawable.toast_warn);
            res = false;
        }
        return res;
    }
    ...
    public void saveRouteRequest(final RouteRequest routeRequest) {
        ...
        new SafeAsyncTask<Boolean>() {
            ...
            public Boolean call() throws Exception {
                if (authToken == null) {
                    serviceProvider.invalidateAuthToken();
                    authToken = serviceProvider.getAuthToken(RouteStepActivity.this);
                }
            ...
            protected void onSuccess(final Boolean isRouteSubmitted) throws Exception {
                ...
                hideProgress();
                if (isRouteSubmitted) {
                    Toaster.showLong(RouteStepActivity.this, getString(R.string.saved_route), R.drawable.toast_info);
                    setResult(RESULT_OK);
                    finish();
                } else {
                    new HandleApiMessages(RouteStepActivity.this, response).showMessages();
                }
            }
        }.execute();
    }
